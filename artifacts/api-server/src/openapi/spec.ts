...
If \`bundleItems\` is omitted or an empty array, the endpoint behaves exactly as before.`,
    ...
    "/sales/stats": {
      get: {
        tags: ["Sales"],
        summary: "Sales statistics for a date range",
        description: "Returns aggregated totals for a shop over a date range. Includes breakdowns by payment method, credit outstanding, on-hold, returns, and `creditCollected` — the sum of payments received against credit-status sales (i.e. debt repayments). Use `start` / `end` (YYYY-MM-DD) to filter. Attendant tokens are automatically scoped to their shop.",
        ...auth(["Admin", "Attendant"]),
        parameters: [
          shopIdQuery,
          { name: "start", in: "query", description: "Start date (YYYY-MM-DD)", schema: { type: "string", format: "date" }, example: "2026-04-01" },
          { name: "end",   in: "query", description: "End date (YYYY-MM-DD)",   schema: { type: "string", format: "date" }, example: "2026-04-30" },
        ],
        responses: dataResp("Sales statistics", {
          totalSales:       { type: "number", description: "Net revenue after returns (grossSales − returns)" },
          grossSales:       { type: "number", description: "Gross revenue of all non-voided/non-returned sales" },
          totalCount:       { type: "integer", description: "Number of non-void sales in the period" },
          returns:          { type: "number", description: "Total refund amount for the period" },
          cashtransactions: { type: "number", description: "Total value of cash-paid completed sales" },
          mpesa:            { type: "number", description: "Total value of M-Pesa completed sales" },
          credit:           { type: "number", description: "Total outstanding balance on credit sales" },
          wallet:           { type: "number", description: "Total value of wallet-paid completed sales" },
          hold:             { type: "number", description: "Total value of on-hold (parked) sales" },
          bank:             { type: "number", description: "Total value of bank-transfer completed sales" },
          creditCollected:  { type: "number", description: "Sum of payments received against credit sales in the period (debt repayments). Identified as salePayments inserted >5 minutes after the originating sale." },
        }),
      },
    },
    "/sales/collected-payments": {
      get: {
        tags: ["Sales"],
        summary: "List collected debt payments",
        description: "Returns a paginated list of payments that were collected against credit sales after the initial sale was recorded. A payment is considered a 'debt collection' if it was inserted more than 5 minutes after its parent sale — this distinguishes repayments from the partial payment recorded at sale time. Each row includes the sale reference, customer details, amount collected, payment method, and the remaining outstanding balance after the payment. Use `start` / `end` to filter by payment date.",
        ...auth(["Admin", "Attendant"]),
        parameters: [
          shopIdQuery,
          { name: "start", in: "query", description: "Start date (YYYY-MM-DD)", schema: { type: "string", format: "date" }, example: "2026-04-01" },
        ...
        responses: listResp("Collected debt payments", {
          id:                { type: "integer", description: "Payment record ID" },
          paid_at:           { type: "string",  format: "date-time", description: "When the payment was received" },
          amount:            { type: "string",  description: "Amount collected (numeric string)" },
          balance:           { type: "string",  description: "Remaining outstanding balance after this payment" },
          payment_type:      { type: "string",  description: "Payment method (Cash | Mpesa | Bank | Wallet …)" },
          payment_reference: { type: "string",  description: "Reference / transaction code (optional)" },
          sale_id:           { type: "integer", description: "Parent sale ID" },
          sale_no:           { type: "string",  description: "Internal sale number" },
          receipt_no:        { type: "string",  description: "Customer-facing receipt number" },
          total_with_discount: { type: "string", description: "Original sale total after discounts" },
          outstanding_balance: { type: "string", description: "Current outstanding balance on the sale" },
          sale_status:       { type: "string",  description: "Current sale status (credit | cashed)" },
          customer_id:       { type: "integer", description: "Customer ID (null for walk-in)" },
          customer_name:     { type: "string",  description: "Customer name (null for walk-in)" },
          customer_phone:    { type: "string",  description: "Customer phone number (null if not set)" },
        }),
      },
    },
    "/sales/{id}": {
