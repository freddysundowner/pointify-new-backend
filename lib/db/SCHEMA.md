...
| status | text | NO | pending / completed / cancelled — default pending |
...
| amount_paid | numeric(14,2) | NO | cached — SUM(purchase_payments.amount). default 0 |
| outstanding_balance | numeric(14,2) | NO | cached — total_amount − amount_paid. default 0 |
...
| line_discount | numeric(14,2) | NO | default 0 |
...
| amount | numeric(14,2) | NO | |
| balance | numeric(14,2) | NO | change/overpay amount — default 0 |
| payment_no | text | YES | auto-generated reference |
| payment_reference | text | YES | M-Pesa code, bank ref, cheque number |
| payment_type | text | NO | cash / mpesa / bank / cheque |
| paid_at | timestamp | NO | defaults to now() |
...
| created_at | timestamp | NO | |
| updated_at | timestamp | NO | **not auto-managed** — API must set `updated_at: new Date()` on every update |
